...
<body>
									...
									<c:choose>
										<c:when test="${not empty communitylists}">
											<c:forEach items="${communitylists}" var="communityitem"
												...
												<tr qunId='${communityitem.getCommunityid()}'cateName='${communityitem.getCategoryname()}' projId='${communityitem.getProjectid()==null?0:communityitem.getProjectid()}' notice='${communityitem.getCommunityNotice()}' >
													<td><i></i></td>
													<td>${status.index +1}</td>
  ...
  <c:otherwise>
    src="<%=contextPath%>/resources/images/q.jpg"
														...
														disabled="disabled">(${communityitem.getCommunityid()})</td>
													<td>
														<div class="selectcatg"><!-- tab-line-input --> 
